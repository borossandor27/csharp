...
namespace NavOnlineSzamlaUploaderRestApi
{
    public class NavApiService
    {
        ...
        public async Task<string> GetExchangeTokenAsync()
        {
            ...
            {
                user = "tech_user",
                password = "sha512-hash-jelszo",
                taxNumber = "12345678",
                requestId = Guid.NewGuid().ToString(),
                timestamp = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ"),
                signature = "aláírás" // SHA3-512 hash: user|requestId|timestamp|kulcs
            };

            var content = new StringContent(JsonSerializer.Serialize(request), Encoding.UTF8, "application/json");
            content.Headers.Add("X-Request-ID", request.requestId);
            content.Headers.Add("X-Software-ID", "SW123");

            var response = await _http.PostAsync($"{_baseUrl}/tokenExchange", content);
            var json = await response.Content.ReadAsStringAsync();

            var result = JsonSerializer.Deserialize<TokenExchangeResponse>(json);
            return result.token;
        }
